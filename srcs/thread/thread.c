...
void		thread_init(void)
{
	...
	cl->device_id = NULL;
	err = clGetDeviceIDs(cl->platform_id, CL_DEVICE_TYPE_DEFAULT, \
		1, &(cl->device_id), &(cl->device_count));
	if (err != CL_SUCCESS)
		die("OpenCL error: device_id");
	cl->context = NULL;
	cl->context = clCreateContext(NULL, 1, &(cl->device_id), \
		opencl_error, NULL, &err);
	if (err != CL_SUCCESS)
		die("OpenCL error: context");
	cl->queue = NULL;
	cl->queue = clCreateCommandQueue(cl->context, cl->device_id, \
		0, &err);
	if (err != CL_SUCCESS)
