...
/*   Updated: 2017/04/14 20:37:18 by stmartin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	program_init(void)
{
	...
	cl->program = clCreateProgramWithSource(cl->context, 1, \
			(const char **)&str, (const size_t *)&size, &err);
	if (err != CL_SUCCESS)
		die("OpenCL error: program source");
	if ((err = clBuildProgram(cl->program, 1, \
					&(cl->device_id), NULL, NULL, NULL)) != CL_SUCCESS)
...
void		cl_render2(t_thread *cl, cl_int err, t_window *window, t_env *env)
{
	if (env->scene.max_object > 0 && cl->mem_object != NULL)
	{
		...
		if ((err = clEnqueueWriteBuffer(cl->queue, cl->mem_object, CL_TRUE, 0, \
				sizeof(t_objects) * env->scene.max_object, env->objects, 0, \
				NULL, NULL)) != CL_SUCCESS)
			die("OpenCL error: Enqueue Write Buffer, Objects");
	}
	else
	{
		if ((err = clSetKernelArg(cl->kernel, 3, sizeof(cl_mem), NULL)) != \
				CL_SUCCESS)
			die("OpenCL error: Kernel set arg 1, Objects");
	}
...
static void	cl_render(void)
{
	...
	if ((err = clEnqueueWriteBuffer(cl->queue, cl->mem_scene, CL_TRUE, 0, \
				sizeof(t_scene), &(env->scene), 0, NULL, NULL)) != CL_SUCCESS)
	...
	if ((err = clEnqueueWriteBuffer(cl->queue, cl->mem_camera, CL_TRUE, 0, \
				sizeof(t_cam), &(env->camera), 0, NULL, NULL)) != CL_SUCCESS)
